...
sequenceDiagram
    ...
    Note over V: health = collateralValue / ((debt + amount) * minRatio)<br/>Assert vault remains healthy

    V->>P: borrow-out(market, amount, borrower)

    Note over P: fee = amount * borrowFeeBps<br/>Split between protocol and LPs<br/>treasuryAccrued += protocolPart

    Note over P: Assert amount <= cash<br/>totalBorrows += amount<br/>cash -= (amount - fee)

    P->>B: Transfer USDC (net of borrow fee)

    V->>CR: increase-stablecoin-debt(market, asset, amount)

    Note over V: borrowPrincipal += amount<br/>Flat debt (no interest)
...
sequenceDiagram
    ...
    Note over V: owed = borrowPrincipal<br/>No interest accrues

    V->>P: repay-in(market, amount, borrower)

    P->>B: transferFrom(USDC)

    Note over P: totalBorrows -= amount<br/>cash += amount

    Note over V: reduce borrowPrincipal
...
```mermaid
sequenceDiagram
    actor K as Health Monitor
    participant L as Liquidation
    participant V as Vault
    participant P as Pool
    participant O as Oracle

    K->>L: liquidate(owner, market, asset)
    ...
    Note over L: Assert health < liquidation threshold

    Note over L: debtToOffset = min(vaultDebt, poolCash + absorbable)

    Note over L: collateralToSeize = base + penalty bonus

    L->>V: liquidate-position(debtToOffset, collateralToSeize)

    Note over V: Reduce borrower debt<br/>Reduce borrower collateral

    V->>P: Transfer seized collateral

    L->>P: distribute-liquidation-reward()

    Note over P: penalty = seized - debtEquivalentCollateral

    Note over P: protocolCut = penalty * protocolShare<br/>treasuryAccrued += protocolCut

    Note over P: poolProduct *= (supplied - loss) / supplied

    Note over P: cumulativeRewardPerToken +=<br/>(seized - protocolCut) / supplied

    L-->>K: OK (debt offset, collateral seized)

    Note over P: LPs later call<br/>claim-collateral-reward()
```
