...
(define-public (deposit (stablecoin-id uint) (stablecoin-token <sip-010-trait>) (amount uint))
  (let (
    ...
    (try! (contract-call? stablecoin-token transfer amount tx-sender (as-contract tx-sender) none))
    (map-set balances
...
(define-public (withdraw (stablecoin-id uint) (stablecoin-token <sip-010-trait>) (amount uint))
  (let (
    ...
    (try! (as-contract (contract-call? stablecoin-token transfer amount tx-sender user none)))
    (map-set balances
...
(define-public (claim-collateral-reward
  ...
  (let (
    ...
    (try! (as-contract (contract-call? collateral-token transfer total-claimable tx-sender user none)))
